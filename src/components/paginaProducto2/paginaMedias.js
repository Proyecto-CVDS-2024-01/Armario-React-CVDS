...
const Medias = () => {
  ...
  useEffect(() => {
    axios.get('http://basecvds.azurewebsites.net/user/prendas')
      .then(response => {
        const filteredProducts = response.data.filter(product => product.tipo === 'MEDIAS');
...
function PaginaMedias() {
  return (
    <div className="PaginaMedias" data-testid="PaginaMedias-1">
      <Top />
      <Medias />
    </div>
  );
